  ...
  <div>
    <el-form ref="loginForm" :model="form" :rules="rules" label-width="80px" class="login-box">
      ...
      <el-form-item label="账号">
        <el-input type="text" placeholder="请输入账号/邮箱" v-model="form.username"/>
      </el-form-item>
          ...
          .then(function (response) {
            ...
            if (response.status === 200) {
              if (response.data.rtCode === 200) {
                ...
                sessionStorage.setItem('token', response.data.bean.token)
                sessionStorage.setItem('usr', JSON.stringify(response.data.data))
                that.$router.push("/")
              } else {
                that.$message({
